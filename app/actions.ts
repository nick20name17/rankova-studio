...
export async function getSectionsByIds(sectionIds: string[]): Promise<NormalizedRecord[]> {
    if (!process.env.AIRTABLE_API_KEY || !process.env.AIRTABLE_BASE_ID) {
        throw new Error("Airtable credentials are not properly configured");
    }
...
export async function fetchAndNormalizePrices(): Promise<ICard[]> {
    ...
    const normalizedPrices = await Promise.all(prices.map(async (priceCard) => {
        // Get only the sections that belong to this price card
        const cardSections = await getSectionsByIds(priceCard?.sections) as any as ISection[];
        
        // Process each section's items individually
        const processedSections = await Promise.all(cardSections.map(async (section) => {
            // If the section has items, fetch only those items
            if (section?.items && Array.isArray(section.items)) {
                const sectionItems = await getItemsByIds(section.items as any as string[]) as unknown as IItem[];
                return {
                    ...section,
                    items: sectionItems
                };
